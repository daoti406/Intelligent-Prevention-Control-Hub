  ...
  if (activeIndex.value === 'dashboard') {
    ...
    if (healthChart) {
      healthChart.setOption({
        ...
        yAxis: { type: 'value' },
        series: [
          { name: '健康率', type: 'line', smooth: true, data: [91.5, 93.2, 95.8, 94.5, 97.6, 98.2, 98.5] },
          { name: '异常率', type: 'line', smooth: true, data: [7.0, 5.3, 3.0, 4.0, 1.4, 0.8, 0.5] },
          { name: '预警率', type: 'line', smooth: true, data: [1.5, 1.5, 1.2, 1.2, 1.0, 1.0, 1.0] }
        ]
      });
    }
    ...
    if (warningChart) {
      warningChart.setOption({
        ...
        legend: { orient: 'vertical', left: 'left', data: ['温度异常', '湿度异常', '行为异常', '其他异常'] },
        series: [{
          name: '预警类型分布',
          type: 'pie',
          radius: ['40%', '70%'],
...
const handleResize = () => {
          ...
          if (activeIndex.value === 'dashboard') {
            ...
            echarts.getInstanceByDom(document.getElementById('humidityChart'))?.resize();
            ...
            echarts.getInstanceByDom(document.getElementById('warningTypeChart'))?.resize();
            echarts.getInstanceByDom(document.getElementById('warningTrendChart'))?.resize();
          }
...
/* 欢迎卡片背景图 */
.welcome-card {
  background: url("/src/assets/images/牛2.jpg");
